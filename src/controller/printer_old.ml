...
let pp_alias ppc cd prt :
  ...
  | al, Some(_,ax,_) ->
     try
       prt ppc (Translation.unconditional_rule_to_p_rule al ax) ;
       incr_real_rule cd
...
let pp_axiom ppc cd prt : quant_var list * axiom * attribute list -> unit =
  ...
  | [Unit _] | [Assoc _] | [Idem _] ->
     ...
     incr_real_rule cd ;
     prt ppc (Translation.equality_axiom_to_p_rule ax)
  | _ -> () (* @TODO *)
