...
let with_prelude ppc cd prt : import list -> unit = fun i_l ->
  set_k_import cd (List.length i_l);
  all_import  := concat_no_duplicate i_l !all_import;
  List.iter (pp_import ppc cd prt [lp_pkg]) !all_import;
  pp_import ppc cd prt (lp_pkg::prelude_path) (prelude_name, []);
