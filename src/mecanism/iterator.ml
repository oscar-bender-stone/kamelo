...
let axiom_cases
      ...
      ((f_implies_ax_constructor     : attribute list -> 'a -> quant_var list * axiom -> 'a),
       (f_implies_ax_initializer     : attribute list -> 'a -> quant_var list * axiom -> 'a),
       (f_implies_ax_projection      : attribute list -> 'a -> quant_var list * axiom -> 'a),
       (f_implies_ax_predicate_true  : attribute list -> 'a -> quant_var list * axiom -> 'a),
  ...
  | Exists _ -> incr_k_exists_ax cd ;
     (match curr_attr with
      | Some (Subsort _) ->
         incr_k_ax_subsort     cd ; f_exists_ax_subsort attr_l acc (qv_l, ax)
      | Some (Functional  _) ->
         incr_k_ax_functional  cd ; f_exists_ax_functional attr_l acc (qv_l, ax)
      | _ -> raise (InternalError "Need to update [axiom_cases], case Exists."))
  | Equals _ -> incr_k_equals_ax cd ;
     (match curr_attr with
      | Some (Assoc _) ->
         incr_k_ax_assoc cd ; f_equals_ax_assoc attr_l acc (qv_l, ax)
      | Some (Comm  _) ->
         incr_k_ax_comm  cd ; f_equals_ax_comm attr_l acc (qv_l, ax)
      | Some (Idem  _) ->
  ...
  | Or _ -> incr_k_or_ax cd ;
     (match curr_attr with
      | Some (Constructor _) ->
         incr_k_or_ax_junk_constructor cd ; f_or_bottom_ax_constructor attr_l acc (qv_l, ax)
      | _ -> raise (InternalError "Need to update [axiom_cases], case Or."))
  | Bottom _ -> incr_k_bottom_ax cd ;
     (match curr_attr with
      | Some (Constructor _) ->
         incr_k_bottom_ax_junk_constructor cd ; f_or_bottom_ax_constructor attr_l acc (qv_l, ax)
      | _ -> raise (InternalError "Need to update [axiom_cases], case Bottom."))
  | Not _ -> incr_k_not_ax cd ;
     (match curr_attr with
      | Some (Constructor _) ->
         incr_k_not_ax_diff_constructor cd ; f_not_ax_constructor attr_l acc (qv_l, ax)
      | _ -> raise (InternalError "Need to update [axiom_cases], case Not."))
  | Implies _ -> incr_k_implies_ax cd ;
     (match curr_attr with
      | Some (Constructor _) ->
         incr_k_ax_same_constructor cd ; f_implies_ax_constructor attr_l acc (qv_l, ax)
      | Some (Initializer _) ->
         incr_k_ax_initializer cd ; f_implies_ax_initializer attr_l acc (qv_l, ax)
      | Some (Projection  _) ->
